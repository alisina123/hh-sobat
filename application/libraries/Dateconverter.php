 ...
 *
 * @package         CodeIgniter
 * @author          Mohammad zafari
 * @license         mohammad.zafari68@gmail.com
 * @since           Version 1.0
 * @filesource
 */
class Dateconverter
{
        ...
        {
            $miladiDate = array();
            $miladiDate = $this->JalaliToGregorian($hshYear,$hshMonth,$hshDay);
            $gd  = $miladiDate[2];
            $gm  = $miladiDate[1];
            $gy  = $miladiDate[0];
            if(strlen($gd)==1)
        ...
        //2 Hamal 1390
        function ToShamsi($grgYear,$grgMonth,$grgDay,$getmonth=false)
        {
            ...
            if($getmonth){
                return $this->monthname_shamsi($month);
            }else{
        ...
        */
        function datecheck($date='',$lang='en',$is_date=false)
        {
            if($lang == 'en' AND $is_date == true)
            {
                return $date;
                //return $y
            }
            elseif($lang == 'en' AND $is_date == false)
            {
                 if($date != '' AND $date != NULL AND $date != 0)
                 {
                    return (date('Y-m-d',$date));
                 }
                 return $date;
            }
            elseif($lang != 'en' AND $is_date == true)
            {
                 if($date != '' AND $date != NULL AND $date != 0)
                 {
                     $exp_date = explode('-',$date);
                     return $this->ToShamsi_short($exp_date[0],$exp_date[1],$exp_date[2]);
                 }
                 return $date;
            ...
            elseif($lang != 'en' AND $is_date == false)
            {
                 ...
                 if($date != '' AND $date != NULL AND $date != 0)
                 {
                     $exp_date = explode('-',date('Y-m-d',$date));
                     return $this->ToShamsi_short($exp_date[0],$exp_date[1],$exp_date[2]);
